...
contract KelePoolStaking is Initializable, UUPSUpgradeable {
    ...
    struct Partner {
        ...
        uint8 feeType; // fee type(0:for every node, 1:for every time)
    }
    ...
    function createValidatorV2(
        ...
        if (role == 0) {
            ...
            if (partner.fee > 0) {
                if (partner.feeType == 0) {
                    // partner fee for every node
                    amount =
                        (_system.fee + partner.fee + DEPOSIT_AMOUNT) *
                        pubkeyCount;
                    partnerFee = partner.fee * pubkeyCount;
                } else if (partner.feeType == 1) {
                    // partner fee for every time
    ...
    function setPartner(
        ...
        if (fee == 0) {
            delete partners[partnerBytes32];
            return;
        }
        partners[partnerBytes32] = Partner(
            fee,
            partnerBytes32,
            recipient,
            feeType
        );
        emit OnPartnerChanged(recipient, partnerBytes32, fee, feeType);
    }
